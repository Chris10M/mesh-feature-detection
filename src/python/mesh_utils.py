...
def calc_A_mixed(vertices, triangles):
    ...
    # print(np.sum(np.sum(mean_curvature_normal_operator, 0), 0))
    # sds
    ...
    m = np.sum(mean_curvature_normal_operator, 1)

    mean_curvature_normal_operator = (area_inv * m.T).T

    # mean_curvature_normal_operator = (
    #     (1 / (2 * A_mixed)) * np.sum(mean_curvature_normal_operator, axis=1).T).T

    # print('A_mixed ', np.sum(A_mixed))
    # print('mean_curvature_normal_operator ', np.sum(mean_curvature_normal_operator, 0))
    # print('m ', np.sum(m, 0))
...
def get_principal_curvatures(vertices, triangles, K_H, K_G):
    numv = vertices.shape[0]
    numt = triangles.shape[0]
...
def main():
    ...
    parser.add_argument(
        "--mode", help="specifies mode for program:\n- 0: For computation\n- 1: For Plotting", type=int, required=False, default=0)
