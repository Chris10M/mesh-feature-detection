...
from test import read_off, calc_A_mixed, get_mean_curvature, get_gaussian_curvature, get_principal_curvatures
...
def extract_mesh_feature(mesh_file):
    f = open(mesh_file)
    ...
    return get_features(vertices, faces)
...
def main():
    ...
    for mesh_file in file_paths:
        ...
        try:
            mesh_weights[mesh_file] = extract_mesh_feature(mesh_file)
        except:
    ...
    # test_mesh_file = 'samplemeshes\sphere_coarse.off'


    distances = list()
    query_mesh_features = extract_mesh_feature(test_mesh_file)
